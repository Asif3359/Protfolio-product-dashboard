  ...
  if (!certificationData) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
        <Typography variant="h6" color="error">Certification not found.</Typography>
      </Box>
    );
  }
    ...
    >
      ...
      <Stack sx={{ mb: 2, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: {xs:'flex-start',sm:'center'} ,gap:2  }}>
        ...
        <Chip
          ...
          label={`Issued: ${new Date(certificationData.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short' })}`}
          size="small"
        />
        {certificationData.expiryDate && (
          <Chip
            icon={<CalendarToday fontSize="small" />}
            label={`Expiry: ${new Date(certificationData.expiryDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short' })}`}
            size="small"
            color="warning"
          />
        )}
      ...
      {certificationData.credentialUrl && certificationData.credentialUrl !== 'N/A' && (
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', mb: 1 }}>
            Credential:
          </Typography>
          <Link href={certificationData.credentialUrl} target="_blank" rel="noopener noreferrer" underline="hover">
            View Certificate
          </Link>
        </Box>
      )}
